...
public class CardContainerIteratorTest extends TestCase {
    ...
    private void testCards(final Iterator<Card> actual, final Iterator<Card> expected) {
    ...
    private void _test__cardContainerIterator_transitions0_effects0_state() {
        _test__cardContainerIterator_transitions0_effects0_state_objectTests0_test();

    }

    private void _test__cardContainerIterator_transitions0_effects0_state_objectTests0_test() {
        try {

            Iterator<Card> _iterator = Collections.<Card>unmodifiableList(CollectionLiterals.<Card>newArrayList(this.s1,
                    this.s2, this.h1, this.h2, this.d1, this.d2, this.c1, this.c2)).iterator();
            this.testCards(this.cardContainerIterator, _iterator);
        } catch (junit.framework.AssertionFailedError error) {
            fail("testCards(cardContainerIterator, #[s1, s2, h1, h2, d1, d2, c1, c2].iterator) failed: "
                    + error.getMessage());
        }
