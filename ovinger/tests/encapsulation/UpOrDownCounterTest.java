...
@JExercise(tests = "", // We don't want to test this element as a whole, since we can test the parts.
...
public class UpOrDownCounterTest extends TestCase {
    ...
    private void testCount(UpOrDownCounter counter, int end, int delta) {
        ...
        while (delta > 0 ? counter.getCounter() < end : counter.getCounter() > end) {
            ...
            assertEquals("When counting from " + i + " the result should be " + expected + ". ", expected,
                    counter.getCounter());
        }
    ...
    @JExercise(tests = "UpOrDownCounter(int,int)", description = "A constructor that initializes the UpOrDownCounter class with the provided start and end value. "
    ...
    public void testUpOrDownCounter() {
        ...
        assertEquals("After instantiating an UpOrDownCounter the counter should be the start value. ", 1,
                counter13.getCounter());
        UpOrDownCounter counter31 = new UpOrDownCounter(3, 1);
        assertEquals("After instantiating an UpOrDownCounter the counter should be the start value. ", 3,
                counter31.getCounter());
    }
    ...
    public void testUpOrDownCounterWithException() {
        ...
        assertTrue("When the start and end values are equal an IllegalArgumentException should be thrown, ",
                iae instanceof IllegalArgumentException);
    }

    @JExercise(tests = "boolean count()", description = "A method that steps the counter, unless it has reached the end value. "
